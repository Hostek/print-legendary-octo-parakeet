...
3.  **Plansza główna**: Wydrukowaną Planszę Główną (Strona 1) kładziemy na środku stołu.
    - _Uwaga: Plansza zawiera zduplikowane słowa obrócone o 180 stopni, aby gracze po obu stronach stołu mogli je łatwo odczytać._
...
W swojej turze Kapitan podaje swoim Operorom wskazówkę składającą się dokładnie z **jednego słowa** i **jednej liczby** (np. _"Ocean: 3"_).
...
## 3. Motywy i własne listy słów

### Korzystanie z gotowych motywów

Możesz wygenerować planszę tematyczną, używając argumentu `--theme`. Motywy automatycznie wybierają słowa w Twoim języku.
...
```bash
python gen.py --lang pl --theme minecraft
```

### Własna lista słów

Jeśli chcesz użyć konkretnej listy słów (np. na urodziny lub konkretną lekcję):

1. Stwórz plik `.txt` z jednym słowem lub frazą w każdej linii.
2. Uruchom generator z argumentem `--words`:

```bash
python gen.py --words moje_slowa.txt
```

_Uwaga: Twoja lista musi zawierać co najmniej 25 słów._
